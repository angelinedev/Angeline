...
const hexToRgb = (hex: string) => {
  ...
  if (h.length === 3)
    h = h
      ...
      .join('');
  if (h.startsWith('hsl')) {
    // Basic HSL to RGB conversion (doesn't handle all cases)
    const hslValues = h.match(/(\d+(\.\d+)?)/g);
    if (!hslValues || hslValues.length < 3) return [255, 255, 255];
    let hu = parseFloat(hslValues[0]);
    let s = parseFloat(hslValues[1]) / 100;
    let l = parseFloat(hslValues[2]) / 100;
    let c = (1 - Math.abs(2 * l - 1)) * s,
        x = c * (1 - Math.abs((hu / 60) % 2 - 1)),
        m = l - c/2,
        r = 0,
        g = 0,
        b = 0;
    if (0 <= hu && hu < 60) {
        r = c; g = x; b = 0;
    } else if (60 <= hu && hu < 120) {
        r = x; g = c; b = 0;
    } else if (120 <= hu && hu < 180) {
        r = 0; g = c; b = x;
    } else if (180 <= hu && hu < 240) {
        r = 0; g = x; b = c;
    } else if (240 <= hu && hu < 300) {
        r = x; g = 0; b = c;
    } else if (300 <= hu && hu < 360) {
        r = c; g = 0; b = x;
    }
...
const TextTrail = ({
  ...
  useEffect(() => {
    ...
    const label = new Mesh(new PlaneGeometry(w*0.8, h*0.8), labelMat);
    scene.add(label);
    ...
    const drawText = () => {
      ...
      const fitScale = Math.min(canvasSize * 0.8 / textWidth, canvasSize * 0.8 / (metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent));
      
      ctx.font = `${fontWeight} ${refSize * fitScale}px "${fontFamily}"`;
    ...
    ref.current.addEventListener('pointermove', onMove as EventListener);
    ...
    const ro = new ResizeObserver(() => {
      ...
      label.geometry = new PlaneGeometry(w*0.8, h*0.8);
      drawText();
    });
    ...
    renderer.setAnimationLoop(() => {
      ...
      const newColor = hexToRgb(textColor || startColor).map(c => c / 255)
      labelMat.uniforms.color.value.set(...newColor);
    ...
    return () => {
      ...
      ro.disconnect();
      if (currentRef?.contains(currentRenderer)) {
         currentRef?.removeChild(currentRenderer);
      }
      renderer.dispose();
      rt0.dispose();
      rt1.dispose();
